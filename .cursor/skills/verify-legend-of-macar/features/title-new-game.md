...
The Book One splash is the original party-tunnel plate (`assets/ui/title_splash.jpg`) plus `Continue`. Continue opens only the Chapter I menu (`title_menu`). That screen cover-bleeds the signed ruby-door hall at `assets/ui/title_menu.jpg` (1920×1080) across the canvas — no gold frame and no black letterbox around a plate. In Chapter I play, the north wall draws `assets/props/prop_rubydoor.png` on `k:'rubydoor'` and `assets/props/prop_dwarfface.png` on `k:'dwarfface'`. New descent / Chapters / Credits and the save Continue flow stay on the menu.
...
- `title-menu-visible` shows the second menu: the Chapter I ruby-door hall full-bleed (door and dwarf face — not the cave-in, not the party splash, not a framed inset), primary `New descent` (blank book) or primary `Continue` plus secondary `New descent` (marked book), plus `Chapters` and `Credits`. Phone pills are at least 48px and sit inside the safe area.
...
- There is no third intro screen. `New descent` enters `play` directly. `Descend` is not a player gate.
- `play-hud` is the seam after New descent. There is no Descend plate.
...
- **Prove menu.** Capture the menu. Run `control-macar browser snapshot --path artifacts/<run>/title-menu-visible.json --aria` and `control-macar browser screenshot --path artifacts/<run>/title-menu-visible.png`. The artifacts show primary `New descent`, not a save `Continue`. The ruby-door hall fills the canvas (Disney's `title_menu.jpg`), not a framed plate, not the cave-in, and not the party-tunnel splash.
- **New descent.** Choose `New descent`. Run `control-macar browser click --name "New descent"` and `control-macar browser wait --scene play --timeout 60`. Scene is `play`. There is no `Descend` plate. A player named MACAR exists.
...
- Screen 1 is `assets/ui/title_splash.jpg` (party tunnel). Screen 2 is only `title_menu`, and its plate is `assets/ui/title_menu.jpg` cover-bled once it decodes. A menu that fills with `title_splash` or the cave-in plate, or that letterboxes the hall inside a gold frame, fails `title-menu-visible`.
...
- New descent does not stop on a chapter plate. Wait for `scene=play`, not a fixed sleep. If world art is still decoding, the canvas holds a full-bleed plate with no Descend button.
