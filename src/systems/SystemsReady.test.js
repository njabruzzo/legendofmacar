...
assert(SR.UPCOMING.indexOf('Navigation')>=0 && SR.UPCOMING.indexOf('PartyOrders')>=0,
  'Navigation and PartyOrders are documented upcoming slots');
...
assert(sysI>=0 && sysI<teI && teI<tapI && tapI<inlineI,
  'load order: SystemsReady → TimedEffects → TapGate → inline play loop');

assert(!/src="src\/systems\/Navigation\.js"/.test(html) &&
  !/src="src\/systems\/PartyOrders\.js"/.test(html),
  'Batch D Navigation / PartyOrders scripts are not shipped yet');
assert(/Future Batch D: Navigation\.js \/ PartyOrders\.js/.test(html),
  'index.html documents the upcoming sync include slots');
...
assert(SR.UPCOMING.every(n=>!SR.has(n)),
  'upcoming Navigation / PartyOrders are not required to start play');
