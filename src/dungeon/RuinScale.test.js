...
assert(/L\.w=140; L\.h=114/.test(ch3), 'ruin map is a larger field of streets');
assert(/rect\(g,82,8,20,14,0\)/.test(ch3) && /rect\(g,40,72,26,16,0\)/.test(ch3),
  'chapter III adds east manors and far-south courts');
assert(/rect\(g,120,40,16,16,0\)/.test(ch3) && /rect\(g,38,98,26,14,0\)/.test(ch3),
  'chapter III grows deep-east and deep-south courts');
...
assert(/rect\(g,20,76,22,12,0\)/.test(ch1) && /rect\(g,102,15,10,14,0\)/.test(ch1),
  'chapter I grows a far-south pantry and far-east drift');

const ch4=html.slice(html.indexOf('if(n===4){'), html.indexOf('if(n===5){'));
assert(/L\.w=132; L\.h=106/.test(ch4), 'chapter IV drowned city is a larger street field');
...
assert(/rect\(g,116,40,14,16,0\)/.test(ch4) && /rect\(g,38,90,24,14,0\)/.test(ch4),
  'chapter IV grows a deep-east quay and deep cistern');

const ch5=html.slice(html.indexOf('if(n===5){'), html.indexOf('sealOuter(L.grid);'));
assert(/L\.w=108; L\.h=98/.test(ch5), 'chapter V temple is a larger hall');
...
assert(/rect\(g,88,14,16,16,0\)/.test(ch5) && /rect\(g,16,80,26,14,0\)/.test(ch5),
  'chapter V grows a far-east chapel and far-south court');
