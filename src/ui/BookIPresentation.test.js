...
const introFn=html.match(/function drawIntro\(g\)\{[\s\S]*?menuBtn\(g,'Descend'/)[0];
assert(/highPlateMetrics\(s, PORT\)/.test(introFn) && /layoutHighPlate\(g, \{/.test(introFn),
  'intro uses the shared high-stack plate layout');
assert(/groupCeil=VH\*m\.ceilK/.test(introFn), 'intro title block starts near the top of the plate');
assert(/titleGap: 28\*s/.test(html), 'intro keeps a generous gap between gold title and flavor');
assert(/titleGapMin: 22\*s/.test(html), 'intro never shrinks the title-to-flavor gap below a wide floor');
assert(/const t0=ceil;/.test(html.match(/function layoutHighPlate\(g, spec\)\{[\s\S]*?function paintHighPlate/)[0]),
...
assert(/layoutHighPlate/.test(html), 'intro copy wraps and shrinks to the band above Descend');
assert(/CH_INTRO\[L\.n\]/.test(html), 'intro draws CH_INTRO, not a clipped inline stub');
assert(/flSize:\(PORT\?18:22\)\*s/.test(introFn), 'intro flavor italic starts larger than the old 14/17 size');
assert(/flSize=Math\.max\(14, flSize-0\.4\)/.test(html), 'flavor italic does not shrink below a readable body size');
assert(/drawCopyVeil/.test(introFn), 'intro uses a soft title-card veil, not a gold box');
assert(!/plate\(g, plateX, plateTop, plateW, plateH/.test(introFn),
...
assert(/ay:0\.52/.test(introFn) && /zoom:1/.test(html),
...
assert(/FONT_DISPLAY/.test(html.match(/function layoutHighPlate\(g, spec\)\{[\s\S]*?function paintHighPlate/)[0]),
  'intro gold title uses the display face');
assert(/MUSIC_LABEL\.chapter/.test(introFn), 'intro title card includes the music credit');
...
function checkIntroBand(name, vw, vh, port){
  ...
  const labGap=8*s;
  const t0=groupCeil;
  const labelY=t0+labSize;
  const titleY=labelY+labGap+titleSize;
  const flavorY=titleY+titleSize*0.12+titleGap;
  const blockH=labSize+labGap+titleSize*1.12+titleGap+flSize*1.32*3+10*s+9*s*1.35;
  ...
  assert(flavorY-titleY>=22*s, name+': generous air between title and flavor ('+(flavorY-titleY).toFixed(1)+'px)');
  assert(flavorY-titleY<=42*s, name+': title-to-flavor gap is not a huge empty hole ('+(flavorY-titleY).toFixed(1)+'px)');
  assert(groupFloor<descendTop, name+': story plate ends above Descend');
  assert(t0===groupCeil, name+': titles stay pinned to the top ceil');
  assert(flavorY<descendTop-8*s, name+': flavor does not crowd Descend');
  assert(t0+blockH<descendTop || flavorY<vh*0.42,
    name+': compact stack stays in the upper plate, not a mid-dark float');
}
