...
assert(/title:splashOk\?''/.test(titleFn) && /THE LEGEND OF MACAR/.test(titleFn),
  'Book One display title stays in the fallback stack; ready splash uses baked LEGEND OF MACAR');
...
assert(/title:splashOk\?''/.test(titleFn) && /flavor:splashOk\?''/.test(titleFn) &&
  /From simple beginnings Macar would rise to become a hero among dwarves\./.test(titleFn),
  'ready splash skips canvas title and flavor so baked type is the only copy');
assert(/label:'B O O K   O N E'/.test(titleFn) && !/label:splashOk\?''/.test(titleFn),
  'BOOK ONE stays on the canvas high plate (not baked in the splash)');
